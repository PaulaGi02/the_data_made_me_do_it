...
import { Droplets, Earth, Info, Heart, ShieldAlert, TrendingUp } from 'lucide-react';
...
// ─── Watercolour botanical SVG illustrations ──────────────────────────────────
...
const WatercolourEucalyptus: React.FC<{ className?: string; opacity?: number }> = ({ className = '', opacity = 1 }) => (
  <svg viewBox="0 0 320 420" xmlns="http://www.w3.org/2000/svg" className={className} style={{ opacity }}>
    <defs>
      <filter id="wce"><feTurbulence type="fractalNoise" baseFrequency="0.04" numOctaves="4" result="n"/>
...
const WatercolourHellebore: React.FC<{ className?: string; opacity?: number }> = ({ className = '', opacity = 1 }) => (
  <svg viewBox="0 0 300 380" xmlns="http://www.w3.org/2000/svg" className={className} style={{ opacity }}>
    <defs>
      <filter id="wch"><feTurbulence type="fractalNoise" baseFrequency="0.045" numOctaves="3" result="n"/>
    ...
    <path d="M145,240 C110,220 80,225 62,205 C80,198 105,194 125,200 C115,185 90,178 76,160 C98,162 120,172 138,185 C130,168 118,150 120,132 C138,148 148,170 150,195"
          fill="#6a9e5a" opacity="0.48" filter="url(#wch)"/>
    <path d="M155,230 C190,210 220,218 238,198 C220,192 195,190 175,196 C185,180 208,175 222,158 C200,160 178,170 162,183 C170,166 182,148 180,130 C162,146 152,168 150,192"
          fill="#72a862" opacity="0.46" filter="url(#wch)"/>
...
const WatercolourLeaves: React.FC<{ className?: string; opacity?: number }> = ({ className = '', opacity = 1 }) => (
  <svg viewBox="0 0 280 340" xmlns="http://www.w3.org/2000/svg" className={className} style={{ opacity }}>
    <defs>
      <filter id="wcl"><feTurbulence type="fractalNoise" baseFrequency="0.042" numOctaves="4" result="n"/>
    ...
    <path d="M115,180 C80,165 50,172 32,150 C22,138 28,118 45,112 C62,106 88,118 108,140"
          fill="#78b078" opacity="0.42" filter="url(#wcl)"/>
    <path d="M122,130 C90,112 68,118 52,96 C44,85 52,66 68,62 C86,57 108,72 122,96"
          fill="#6eaa6e" opacity="0.40" filter="url(#wcl)"/>
    <path d="M118,78  C95,58  80,62  66,42  C60,32  66,14  82,10  C98,6   116,22  120,48"
          fill="#82be82" opacity="0.38" filter="url(#wcl)"/>
    <path d="M140,200 C175,182 205,188 222,164 C232,150 225,130 208,125 C190,120 166,134 148,156"
          fill="#80b880" opacity="0.44" filter="url(#wcl)"/>
    <path d="M138,148 C170,128 198,134 214,110 C222,98 214,78  198,74  C180,70  158,84  142,108"
          fill="#70ae70" opacity="0.42" filter="url(#wcl)"/>
    <path d="M136,98  C162,76  186,80  200,58  C206,46  198,28  184,26  C168,22  148,38  136,60"
          fill="#86c286" opacity="0.38" filter="url(#wcl)"/>
...
const WatercolourBranch: React.FC<{ className?: string; opacity?: number }> = ({ className = '', opacity = 1 }) => (
  <svg viewBox="0 0 360 240" xmlns="http://www.w3.org/2000/svg" className={className} style={{ opacity }}>
    <defs>
      <filter id="wcb"><feTurbulence type="fractalNoise" baseFrequency="0.038" numOctaves="3" result="n"/>
      <feDisplacementMap in="SourceGraphic" in2="n" scale="8" xChannelSelector="R" yChannelSelector="G"/></filter>
    </defs>
...
// ─── Botanical corner decorations for chart frames ────────────────────────────
const CORNER_SVGS = [
  // 0 — fern sprig
  (color: string) => (
...
const ChartCornerPlant: React.FC<{ position: 'tl'|'tr'|'bl'|'br'; variant: number; dark?: boolean }> = ({
  position, variant, dark = false,
}) => {
...
interface NarrativeRowProps {
  ...
  flip?: boolean; dark?: boolean; delay?: number;
}
...
const NarrativeRow: React.FC<NarrativeRowProps> = ({ label, heading, body, Illustration, flip=false, dark=false, delay=0 }) => {
  ...
  const textCol = (
    ...
    >
      ...
      <p className={`font-body text-mg md:text-xl leading-relaxed ${dark ? 'text-cream/80' : 'text-ink/80'}`}
...
interface SubSectionProps {
  content: string; type: 'explanation'|'context'|'personal';
  align: 'left'|'right'; title: string; subtitle?: string; category: StoryCategory;
}
      ...
      <div className="space-y-6">
        <div className="flex items-center gap-4">
          <div className={`w-14 h-14 rounded-full flex items-center justify-center shadow-inner ${
            type==='explanation'?'bg-sg text-dg':type==='context'?'bg-mg text-white':'bg-lg text-dg'}`}>
        ...
        <p className="text-xl md:text-2xl leading-relaxed font-body italic"
           dangerouslySetInnerHTML={{ __html: type==='personal'?`"${content}"`:content }}/>
...
const StorySectionWrapper: React.FC<{ section: StorySection; index: number; onInView:(id:number)=>void }> = ({
  section, index, onInView,
}) => {
  ...
  useEffect(()=>{ if(isInView) onInView(section.visualId); },[isInView,section.visualId,onInView]);
  ...
  return (
    <div ref={containerRef} className={`relative z-10 ${isSingle?'h-[400vh]':'h-[900vh]'} ${bgClass}`}>
          ...
          <>
            {section.paragraphs&&(
              <>
                ...
                <div className="pointer-events-auto">
                  <SubSection content={section.paragraphs.personal} type="personal" align="right"
                    title={section.title} category={section.category}/>
                </div>
              </>
            )}
...
const HeroLeaf: React.FC<{className?:string;rotate?:number;delay?:number}> = ({className,rotate=0,delay=0})=>(
  <motion.div initial={{opacity:0,scale:0.8,rotate:rotate-10}} whileInView={{opacity:0.15,scale:1,rotate}}
    ...
    <svg viewBox="0 0 200 200" className="w-full h-full fill-dg">
      ...
      <path d="M100,20 L100,180 M100,60 L140,80 M100,90 L60,110 M100,120 L140,140 M100,150 L60,170"
    ...
    <div className="relative bg-cream min-h-screen font-body">
      ...
      <section className="h-screen flex flex-col items-center justify-center text-center px-4 relative overflow-hidden bg-dg text-cream z-50">
        ...
        <motion.div initial={{opacity:0,y:40}} animate={{opacity:1,y:0}}
          ...
          <motion.button onClick={()=>introRef.current?.scrollIntoView({behavior:'smooth'})}
      ...
      <div ref={introRef}>
        <NarrativeRow
          ...
          body="I was already interested in sustainability and I noticed that my diet plan could play a bigger role than i had imagined. Looking at the numbers made me realize, my diet makes a differnece."
        ...
        <NarrativeRow
          ...
          body="Each section shows a different dataset showcasing the staus quo (global scale), the environmental- and the health impact. Scroll through them and watch whether a pattern emerges. I'll tell you what I found, but you can look for yourself."
      ...
      <NarrativeRow
        label="Key takeaways"
        heading="It was never one chart."
        body="No single statistic converted me. What changed my mind was the consistency. Twelve datasets with three independent dimensions of the status quo, environmental impact and the public health problems. We could see that meat consumption is not evenly distributed and meat has in several categories a bad influence on our environmeant as well our public health."
        Illustration={WatercolourLeaves}
        flip={false} dark={true} delay={0}
      />
      <NarrativeRow
        label="Where I stand now"
        heading="I do not think everyone should turn vegan."
        body="I'm asking you to look at the same numbers I looked at and draw your own conclusion. The data is public. The pattern is there whether I narrate it or not. And while not everyone should turn vegan I think it is important to consume meat with more awareness."
        Illustration={WatercolourBranch}
        flip={true} dark={false} delay={0.1}
      />
      ...
      <section className="min-h-screen flex flex-col items-center justify-center text-center px-8 bg-dg text-cream relative overflow-hidden py-20">
        ...
        <motion.div initial={{opacity:0,scale:0.95}} whileInView={{opacity:1,scale:1}}
          transition={{duration:1.2}} className="max-w-4xl z-10">
