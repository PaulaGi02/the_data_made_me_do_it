...
const BotanicalElement: React.FC<{
    className?: string;
    delay?: number;
    rotate?: number;
    variant?: 'leaf' | 'vine' | 'flower';
}> = ({className, delay = 0, rotate = 0, variant = 'leaf'}) => (
    <motion.div
    ...
    >
        <svg viewBox="0 0 200 200" className="w-full h-full fill-dg">
            {variant === 'leaf' && (
                <>
                    <path
                        d="M100,20 C120,50 160,70 160,110 C160,150 130,180 100,180 C70,180 40,150 40,110 C40,70 80,50 100,20 Z"/>
                    <path
                        d="M100,20 L100,180 M100,60 L140,80 M100,90 L60,110 M100,120 L140,140 M100,150 L60,170"
                        stroke="white"
                        strokeWidth="2"
                        fill="none"
                    />
            ...
            {variant === 'vine' && (
                <path
                    d="M100,200 C100,150 150,150 150,100 C150,50 50,50 50,0"
                    stroke="currentColor"
                    strokeWidth="4"
                    fill="none"
            ...
            {variant === 'flower' && (
                <path
                    d="M100,100 m-40,0 a40,40 0 1,0 80,0 a40,40 0 1,0 -80,0 M100,60 a40,40 0 1,0 0,80 a40,40 0 1,0 0,-80"/>
            )}
        </svg>
    </motion.div>
);
...
const SubSection: React.FC<SubSectionProps> = ({
    ...
    return (
        <div
            className={`h-[200vh] flex items-center px-6 md:px-20 relative z-20 ${
        ...
        >
            ...
            >
                <div className="space-y-6">
                    ...
                    <p
                        className="text-xl md:text-2xl leading-relaxed font-body italic"
                        dangerouslySetInnerHTML={{
                            __html: type === 'personal' ? `"${content}"` : content
                        }}
                    />
...
const StorySectionWrapper: React.FC<{
    ...
    return (
        <div ref={containerRef} className={`relative z-10 ${isSingle ? 'h-[400vh]' : 'h-[900vh]'}`}>
            {/* Sticky Chart Container */}
            <div
                className="sticky top-0 h-screen w-full flex items-center justify-center p-4 md:p-8 lg:p-12 z-0 overflow-hidden pointer-events-none">
                <motion.div
                    style={{opacity: chartOpacity, scale: chartScale, y: chartY}}
                    className="w-full h-full max-w-[92vw] max-h-[85vh] relative rounded-[1.5rem] overflow-hidden shadow-[0_0_150px_rgba(0,0,0,0.2)] border-[12px] border-cream-dark bg-white pointer-events-auto"
                >
                    <TableauViz vizUrl={section.tableauSheet}/>
                    <div
                        className="absolute top-10 left-10 w-20 h-20 border-t-2 border-l-2 border-dg/5 rounded-tl-[1rem] pointer-events-none"/>
                    <div
                        className="absolute bottom-10 right-10 w-20 h-20 border-b-2 border-r-2 border-dg/5 rounded-br-[1rem] pointer-events-none"/>
                </motion.div>
            </div>
...
export default function App() {
    ...
    return (
        <div className="relative bg-cream min-h-screen font-body">
            ...
            {/* Hero Section */}
            <section
                className="h-screen flex flex-col items-center justify-center text-center px-4 relative overflow-hidden bg-dg text-cream z-50">
                <BotanicalElement className="w-96 h-96 -top-20 -left-20 opacity-20" rotate={-15}/>
                <BotanicalElement
                    className="w-[30rem] h-[30rem] -bottom-40 -right-20 opacity-20"
                    rotate={160}
                />
            ...
            <div className="relative" ref={firstSectionRef}>
                {storyData.map((section) => (
                    <StorySectionWrapper key={section.id} section={section} onInView={() => {
                    }}/>
                ))}
            </div>
            ...
            {/* Conclusion Section */}
            <section
                className="min-h-screen flex flex-col items-center justify-center text-center px-8 bg-dg text-cream relative overflow-hidden py-20">
                <BotanicalElement
                    className="w-[40rem] h-[40rem] top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 opacity-10"
                    rotate={30}
                />
                ...
                >
                    <h2 className="text-7xl md:text-9xl font-black tracking-tighter mb-12 uppercase font-serif leading-none">
                        Where do you <br/>
                        <span className="text-lg italic font-normal lowercase font-serif block mt-4">
              stand?
            </span>
                    </h2>
                    <p className="text-2xl md:text-4xl font-medium opacity-80 mb-20 leading-relaxed font-body italic max-w-2xl mx-auto">
                        It wasn't one chart. It was the weight of them all. The data built a case my conscience
                ...
                <footer className="absolute bottom-10 w-full text-center">
                    ...
                    <p className="text-xs opacity-40 font-bold tracking-[0.5em] uppercase">
                        Paula Giensch &bull; 2026 &bull; Data Driven Storytelling
                    </p>
                </footer>
            </section>
        </div>
